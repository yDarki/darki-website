...
export async function onRequest(context) {
  ...
  if (url.searchParams.has('cron')) {
    ...
    for (const src of SOURCES) {
      ...
      let _icon = null, _sname = null; if (src.guildId) { try { const _g = await fetch('https://discord.com/api/v10/guilds/' + src.guildId, { headers: { Authorization: 'Bot ' + bot } }); if (_g.ok) { const _gj = await _g.json(); if (_gj && _gj.icon) _icon = 'https://cdn.discordapp.com/icons/' + src.guildId + '/' + _gj.icon + '.png?size=64'; if (_gj && _gj.name) _sname = _gj.name; } } catch (e) {} } const _link = src.invite || (src.guildId ? ('https://discord.com/channels/' + src.guildId + '/' + src.channelId) : ''); out.push({ name: _sname || src.name || 'Market', channelId: src.channelId, icon: _icon, link: _link, updated: Date.now(), sourceTs: res.ts || null, spawners: parsePrices(res.text) });
    }
