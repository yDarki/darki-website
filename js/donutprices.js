...
(function(){
  ...
  function svgGraph(points, W, H, opts){
    ...
    var _gg=[]; for(var gk=1;gk<points.length;gk++) _gg.push(points[gk].x-points[gk-1].x); var _gs=_gg.slice().sort(function(a,b){return a-b;}); var _med=_gs.length?_gs[Math.floor(_gs.length/2)]:0; var GAPMS=Math.max(1800000, _med*4);
  ...
  function drawInto(el, big){
    ...
    el.innerHTML=svgGraph(pts,W,H,{x0:x0,x1:x1,ticks:ticks});
    wireHits(el);
  }
